...
|-------|----------|-------|
| RPI Shield PNG | Embedded as base64 data URL in HTML | 318x284px, extracted from SVG wrapper |
| RPI Shield SVG (DO NOT USE FOR PRINT) | `apps/prodash/public/rpi-shield.svg` | Fake SVG — wraps a low-res PNG in a 61x53 viewBox. Causes clipping + blur in PDF |
| HTML Source | `apps/prodash/public/RPI Retirement Portfolio Insights.html` | Print-ready |
| Google Fonts | `Inter` weights 300-800 | Loaded via Google Fonts CDN |

### Logo Fix for Print (CRITICAL — Apply to ALL RPI Print Materials)

**The Problem:** `RPI-3.svg` from retireprotected.com is NOT a true vector SVG. It's a 318x284 raster PNG stuffed inside an SVG container with a tiny 61x53 viewBox. When Chrome renders this to PDF at 300 DPI:
- The SVG viewBox clips the image (cuts off top/bottom of laurel wreaths)
- The downscaled dimensions produce blur/pixelation

**The Fix:** Extract the raw PNG data from inside the SVG and embed it directly as a `data:image/png;base64,...` URL in the `<img>` tag. This bypasses the SVG wrapper entirely and gives the PDF renderer direct access to all 318x284 pixels.

**Extraction script:**
```bash
python3 -c "
import base64, re
with open('rpi-shield.svg', 'r') as f:
    svg = f.read()
match = re.search(r'(data:image/png;base64,[A-Za-z0-9+/=]+)', svg)
print(match.group(1))  # Use this as the img src value
"
```

**Then in HTML:** `<img src="data:image/png;base64,iVBORw0KGgo..." alt="RPI">`

**This applies to ALL future RPI print materials using the shield logo.** Until a true vector SVG of the shield exists, always use the extracted PNG data URL for print output. The SVG URL is fine for screen-only rendering.
