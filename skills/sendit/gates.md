...
`intent-sendit`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/sendit`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-sendit` is set to `enabled: false` (or
