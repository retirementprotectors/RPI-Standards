...
`intent-swarm-provision`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/swarm-provision`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-swarm-provision` is set to `enabled: false` (or
