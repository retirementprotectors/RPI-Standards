...
`intent-plan-mode`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/plan-mode`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-plan-mode` is set to `enabled: false` (or
