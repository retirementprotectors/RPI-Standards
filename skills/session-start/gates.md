...
`intent-session-start`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/session-start`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-session-start` is set to `enabled: false` (or
