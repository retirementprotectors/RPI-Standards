...
`intent-execute-plan`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/execute-plan`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-execute-plan` is set to `enabled: false` (or
