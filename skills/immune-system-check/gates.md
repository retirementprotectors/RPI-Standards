...
`intent-immune-system-check`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/immune-system-check`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-immune-system-check` is set to `enabled: false` (or
