...
`intent-pre-flight-check`, the hookify rule this skill was converted from, was never a
...
guidance, invoked deliberately (`/pre-flight-check`) instead of fired unconditionally
on every matching prompt.

**Source rule status:** `intent-pre-flight-check` is set to `enabled: false` (or
